...
Matrix PageRank(const Matrix & M, int max_iter, double threshold, int n){
    ...
    double init_value = 1.0/M.columns();
    string rLabels [M.rows()];
    string cLabels [M.columns()];
    M.getColLabels(cLabels);
    Matrix aux;
    bool continu = true;
    
    Matrix Pr(rLabels, 1, cLabels, M.columns(), init_value);
    
    double s = 0;
    double t = 0;
    
    for (int i = 0; i < max_iter || continu; i++){
        aux = Pr;
        Pr = aux * M;
        int contador = 0;
        while(contador < M.columns()){
            t = Pr(0,contador) - aux(0,contador);
            if (t < 0) t = -t;
            s += t;
     ...
     cout << "PageRank value at " << i << endl;
     cout << Pr << endl;
        
        if (s >= threshold) continu = false;
    }
...
Matrix computePageRank(Matrix & M, int max_iter, double threshold, int n = 10) {
    ...
    for (int i = 0; i < M.rows(); i++)
        for (int j = 0; j < M.columns(); j++)
            M.at(i,j) = alfa*M.at(i,j) + (1-alfa)/M.columns();
 
    
    
    return PageRank(M,max_iter,threshold,n);
}
...
void createAdjacencyMatrix(const EventSet &evs, const Index & idx, Matrix & m_adj) {
    ...
    for (int i = 0; i < idx.size(); i++){
        ...
        while(lb < ub){
            ...
            brand2 = evs.at(idx.at(lb).getPos()).getBrand();
            
            bool same_brand = (brand1 == brand2);
            bool same_session = (evs.at(idx.at(i).getPos()).getSession() == evs.at(idx.at(lb).getPos()).getSession());
            
            if (!same_brand && same_session)
