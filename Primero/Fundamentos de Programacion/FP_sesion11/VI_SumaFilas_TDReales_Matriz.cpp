...
public:
	...
	/*****************************************************************/
	// Método que se encarga de crear un string que pinte la suma de todos los
	// numeros de la fila. Si los datos introducidos son erróneos, también se 
	// pintará.
	//
	// PRE: 0 <= indice_fila < filas_utilizadas
	
	string PintaSumaFilaReales (int indice_fila){
		...
		if(!DatosCorrectosSumaFila(indice_fila)){
			cadena = "-Error al introducir los datos-";
		}	
		else{
			do{
				for(int i = 1; i <= matriz_privada[indice_fila][pos_entero_fila]; i++){
									
					suma = matriz_privada[indice_fila][pos_entero_fila + i] + suma;
				}
				cadena = cadena + " " + to_string(suma) + " ";
				pos_entero_fila += matriz_privada[indice_fila][pos_entero_fila]+1;
				suma = 0;
...
private:
	...
	bool DatosCorrectosSumaFila (int indice_fila){
		...
		do{
			int num_entero_principal = matriz_privada[indice_fila][pos];
		...
		}while(pos < ColumnasUtilizadas(indice_fila));
...
int main(){
	...
	/***********************************************************************/
	// Pedir la matriz que se va a sumar
	lector.SetTitulo("Introduzca la fila cuyos datos se quieren sumar: ");
	int num_fila = lector.LeeEnteroEnRango(1, num_filas_matriz);
	
	// Mostrar el resultado
	cout<<"Resultado: "<<matriz.PintaSumaFilaReales(num_fila-1);
