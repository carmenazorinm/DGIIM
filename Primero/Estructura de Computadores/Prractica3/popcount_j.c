	...
	long result = 0; 
  	...
  	for(i = 0; i < len; i++){
	   ...
	   asm("\n" 
               ...
               "shr %[x] \n\t"  // LSB en CF
               "adc $0, %[r] \n\t" //añadir acarreo a r (añadir a r si se ha encontrado un 1)
               "test %[x],%[x] \n\t" // mirar si x es cero
               "jnz ini3" // si x no es cero, empezamos
...
int popcount4(unsigned* array, size_t len) 
{
	...
	long result = 0; 
  	...
  	for(i = 0; i < len; i++){
	   ...
	   "clc  \n\t"  // limpia el acarreo
      	   "ini4:    \n\t" 
	   "adc $0, %[r]  \n\t"  // guarda el acarreo en r
    	   "shr %[x] \n\t" // desplaza a la derecha y actualiza CF
	   "jne ini4 \n\t" // si x!=0, vuelvea empezar
	   "adc $0, %[r] \n\t" //guarda el último acarreo
