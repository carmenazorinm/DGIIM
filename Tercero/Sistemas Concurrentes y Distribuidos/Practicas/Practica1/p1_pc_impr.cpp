...
int vec[num_items] = {0}; //vec intermedio
...
void funcion_hebra_consumidora(  )
{
   for( unsigned i = 0 ; i < num_items ; i++ )
   {
      ...
      celdas_ocupadas--;
      primera_ocupada++; //ahora la casilla first ya se ha leído, será la siguiente
...
void funcion_impresora( )
{
  for(int i = 0; i < num_items/5; i++){
    sem_wait( imprime );
    cout << "Casillas ocupadas = " << celdas_ocupadas << endl;
    sem_signal( continua );
  }
}
