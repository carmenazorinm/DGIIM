...
void  funcion_hebra_productora(  )
{
   for( unsigned i = 0 ; i < num_items ; i++ )
   {
      ...
      vec[ocupados] = dato; //se añade el dato a la primera casilla no ocupada
      ocupados++; //ahora hay una casilla más ocupada
...
void funcion_hebra_consumidora(  )
{
   for( unsigned i = 0 ; i < num_items ; i++ )
   {
      ...
      sem_wait( ocupadas );
      dato = vec[ocupados-1]; //leer la última casilla ocupada
      ocupados--; //ahora hay una casilla menos ocupada
