...
const int n_productoras = 1;
const int n_consumidoras = 1;
...
bool ProdConsSU1::metodo_impresora(){
  ...
  if(num - num_anterior == 0){
    impresora.wait();
  }

  bool result;
  if(num == n_multiplos_5)
    result = false;
  else{
    cout << "Todavía quedan " << n_multiplos_5 - num <<" multiplos de 5 por insertar..." << endl;
    result = true;
  }
...
int ProdConsSU1::leer(  )
{
   ...
   if ( primera_ocupada == primera_libre  || primera_ocupada == -1)
      ocupadas.wait();
...
void funcion_impresora(MRef<ProdConsSU1> monitor)
{
  bool sigue = true;
  while(sigue){
    sigue = monitor->metodo_impresora();
  }
}
// -----------------------------------------------------------------------------
...
int main()
{
   cout << "--------------------------------------------------------------------" << endl
    ...
    // }
    thread productora(funcion_hebra_productora, monitor,0);
    thread consumidora(funcion_hebra_consumidora, monitor);
    thread impresora(funcion_impresora, monitor);
    impresora.join();
    productora.join();
    consumidora.join();
