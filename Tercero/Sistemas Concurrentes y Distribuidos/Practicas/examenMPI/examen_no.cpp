...
void funcion_npc( )
{
  ...
  int etiqueta;
  int kilos_peach[2]={aleatorio<1,5>(),aleatorio<1,5>()};
  int kilos_mario[2]={aleatorio<1,5>(),aleatorio<1,5>()};
  int n_veces = 0;
  ...
  //mando a peach
  	for (int i=0; i<2; i++){
  		kilos_peach[i]=aleatorio<1,5>();
  	}
  	for (int i=0; i<2; i++){
  		kilos_mario[i]=aleatorio<1,5>();
  	}
  	int tartas_a_mandar_peach=aleatorio<1,2>();
  	...
  	int tartas_a_mandar_mario=aleatorio<1,2>();
  	  	// cout << "Se mandan valores" << endl;
