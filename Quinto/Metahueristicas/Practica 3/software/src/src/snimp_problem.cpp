...
tSolution Snimp::mutate(tSolution &sol, double porcentaje) {
    tSolution mutated = sol;
    set<int> currentNodes(sol.begin(), sol.end()); // Para acceso rápido
    vector<int> allNodes = nodos();

    // Crear lista de nodos no usados
    vector<int> unusedNodes;
    for (int node : allNodes) {
        if (!currentNodes.count(node)) {
            unusedNodes.push_back(node);
    ...
    if (unusedNodes.empty()) return mutated; // Nada que hacer

    // Número de genes a mutar según el porcentaje
    int numMutaciones = max(1, int(solSize * porcentaje));

    // Asegurar que no mutamos más de lo que se puede
    numMutaciones = min(numMutaciones, min((int)solSize, (int)unusedNodes.size()));

    // Elegir aleatoriamente las posiciones a mutar
    vector<int> posiciones(solSize);
    iota(posiciones.begin(), posiciones.end(), 0);
    random_shuffle(posiciones.begin(), posiciones.end());

    // Elegir nodos nuevos al azar
    random_shuffle(unusedNodes.begin(), unusedNodes.end());

    for (int i = 0; i < numMutaciones; ++i) {
        int pos = posiciones[i];
        int newNode = unusedNodes[i];
        mutated[pos] = newNode;
    }
