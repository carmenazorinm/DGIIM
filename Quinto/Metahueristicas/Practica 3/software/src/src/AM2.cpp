...
ResultMH AM2::optimize(Problem *problem, int maxevals) {
    const int popSize = 30;
    const double Pc = 0.7;
    const double Pm = 0.1;
    const double pLS = 0.1; // Probabilidad de aplicar BL a un cromosoma
    ...
    while (evals < maxevals) {
        //if(evals %40 == 0) cout << "Eval: " << evals << "Mejor fitness AM2: " << bestFit << endl;
        generation++;
        ...
        // Aplicar BL cada 10 generaciones sobre un subconjunto de hijos
        if (generation % generacionesBL == 0) {
            for (int i = 0; i < popSize && evals < maxevals; ++i) {
                if (Random::get<double>(0.0, 1.0) <= pLS) {
                    BLsmall bl;
